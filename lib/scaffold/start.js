...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for privyd is necessary for this upgrade with the "reindex=1" privy.conf option. \n' +
                 'There are changes necessary in both privy.conf and bitcore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/privyd')
            }
          }
        }
      };
 ...
 * @param {String} options.config.network - 'mainnet', 'testnet' or 'regtest
